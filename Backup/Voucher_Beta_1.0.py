...
def make_printed():
    try:
        ...
        unprinted_codes = [(row[0], i + 1) for i, row in enumerate(values[1:]) if len(row) >= 3 and not row[2]]
        ...
        with Image.open(final_image_filename) as final_img:
            ...
            for code in codes:
                ...
                coupon_width, coupon_height = 408, 650
                positions = [
                    (0, 0), (408, 0), (816, 0),
...
def activate_and_email(name, email_address):
    try:
        ...
        code_without_asterisk = code.replace('*', '')
        barcode_filename = f"barcode_{code_without_asterisk}"
        generate_barcode(code_without_asterisk, barcode_filename)

        time.sleep(1)

        image_filename = os.path.join(DEFAULT_DIR, 'standaard2.png')
        output_path = os.path.join(OUTPUT_DIR, f'Coupon_email_{code_without_asterisk}.png')

        if not os.path.exists(image_filename):
            print("Error: Image file not found.")
            return
        barcode_filepath = os.path.join(OUTPUT_DIR, f"{barcode_filename}.png")
