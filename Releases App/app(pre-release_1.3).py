...
class LoginScreen(Screen):
    ...
    def show_error_popup(self, title, message):
        # Show an error popup with the provided title and message
        show_success_popup(title, message)  # Assuming show_success_popup is defined elsewhere
...
class EmailScreen(Screen):
    ...
    def activate_voucher(self, instance):
        ...
        self.unprinted_codes = self.fetch_unprinted_codes()  # Fetch codes every time activation is attempted
        Clock.schedule_once(self._activate_voucher, 0.1)

    def _activate_voucher(self, dt):
        try:
            self.valid_codes = [code for code in self.unprinted_codes if self.is_valid_for_activation(code)]
            if self.valid_codes:
                selected_code = random.choice(self.valid_codes)
                self.activate_code_in_sheet(selected_code)
                ...
                self.show_error_popup("Activation Error", "No valid codes available for activation.")
        except Exception as e:
            print("Error:", e)
        finally:
            self.activate_button.text = "Activate Voucher"

    def is_valid_for_activation(self, code):
        return code.startswith("*") and not code.endswith("#")
    ...
    def fetch_unprinted_codes(self):
        try:
            ...
            print("Error fetching unprinted codes:", e)
            return []

    def activate_code_in_sheet(self, code):
        try:
            creds = self.get_credentials()
            if creds:
                client = gspread.authorize(creds)
...
Het Fietsstation"""
        ...
        webbrowser.open(f"mailto:{self.email_input.text}?subject=Uw Vouchercode&body={encoded_email_body}")
