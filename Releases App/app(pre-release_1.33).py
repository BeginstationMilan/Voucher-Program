...
def make_printed(credentials_file_path=None):
    try:
        ...
        with Image.open(final_image_filename) as final_img:
            ...
            try:
                open("//storage//emulated//0//Download//Voucher-Program-main//Output//Coupon_list.pdf")
...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        Window.clearcolor = get_color_from_hex('#00BFFF')  # Consistent bright blue background

        layout = BoxLayout(orientation='vertical', spacing=10, padding=20)

        # Title for the login screen
        title_label = Label(text="Voucher Login", font_size='34sp', color=get_color_from_hex('#FFFFFF'))
        layout.add_widget(title_label)

        self.username_input = TextInput(hint_text='Enter username', multiline=False, font_size='23sp', size_hint_y=None, height=120)
        self.password_input = TextInput(hint_text='Enter password', multiline=False, password=True, font_size='23sp', size_hint_y=None, height=120)
        self.login_button = Button(text='Login', on_press=self.login, background_color=get_color_from_hex('#3D7CC9'), font_size='16sp', size_hint_y=None, height=100)
