...
def make_printed(credentials_file_path=None):
    try:
        ...
        with Image.open(final_image_filename) as final_img:
            ...
            # Attempt to open the PDF using the specified path
            try:
                open("//storage//emulated//0//Downloads//Voucher-Program-main//output//Coupon_list.pdf")
                print(f"PDF opened: {coupon_list_pdf}")
            except Exception as e:
                print(f"Failed to open PDF: {str(e)}")
                show_error_popup("Error", "PDF saved but could not be opened automatically.")
    except Exception as e:
