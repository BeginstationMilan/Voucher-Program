...
class LoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.username_input = TextInput(hint_text='Enter username', multiline=False, font_size=20)
        self.password_input = TextInput(hint_text='Enter password', multiline=False, password=True, font_size=20)
        self.login_button = Button(text='Login', on_press=self.login, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
...
class VoucherApp(App):
    def build(self):
        ...
        self.email_screen = EmailScreen(name='email')  # Add the EmailScreen
        ...
        self.screen_manager.add_widget(self.email_screen)  # Add the EmailScreen
    ...
    def __init__(self, **kwargs):
        ...
        self.buttons = {
            'Generate': Button(text='Generate Vouchers', on_press=self.switch_to_generate,
                               background_color=button_color, font_size=20),
...
class GenerateScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.generate_input = TextInput(hint_text='Enter number of vouchers to generate', multiline=False, font_size=20)
        self.generate_button = Button(text='Generate Vouchers', on_press=self.generate_vouchers, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main, background_color=[0.7, 0.3, 0.3, 1], font_size=20)
...
class EmailScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.label = Label(text="Enter email address:")
        self.email_input = TextInput(hint_text="Email")
        self.activate_button = Button(text="Activate Voucher", size_hint=(1, 0.5))
        self.return_button = Button(text="Return to Main Menu", size_hint=(1, 0.5))

        self.activate_button.bind(on_release=self.activate_and_email_voucher)
        self.return_button.bind(on_release=self.return_to_main_menu)
    ...
    def activate_and_email_voucher(self, instance):
        self.activate_button.text = "Activating..."
        Clock.schedule_once(lambda dt: self.process_voucher_activation_and_email(), 0.1)

    def process_voucher_activation_and_email(self):
        try:
            unprinted_codes = self.fetch_unprinted_codes()
            valid_codes = [code for code in unprinted_codes if self.is_valid_for_activation(code)]

            if valid_codes:
                selected_code = random.choice(valid_codes)
                self.activate_code_in_sheet(selected_code, self)
                self.email_voucher_code(selected_code)
                print(f"Code activated and emailed: {selected_code}")
            else:
                print("No valid codes available for activation.")
        except Exception as e:
            print("Error:", e)
        finally:
    ...
    @staticmethod
    def activate_code_in_sheet(code_data, self):
        try:
            code = code_data[0]
            ...
            if creds:
                ...
                sheet = client.open('voucher-barcode').sheet1
                cell = sheet.find(code)
                if cell and cell.value.startswith("*") and not sheet.cell(cell.row, 3).value:
                    sheet.update_cell(cell.row, 1, code[1:])
                    sheet.update_cell(cell.row, 3, "Printed")
                    expiration_date = (datetime.datetime.now() + datetime.timedelta(days=365)).strftime("%m/%d/%Y")
                    sheet.update_cell(cell.row, 4, expiration_date)
                    print(f"Code activated and marked as printed. Valid until {expiration_date}.")
                else:
                    print("Code is not valid for activation.")
            else:
                print("Credentials not available.")
        except Exception as e:
            print("Error activating code:", e)

    def fetch_unprinted_codes(self):
        creds = self.get_credentials()
        if creds:
            client = gspread.authorize(creds)
            sheet = client.open('voucher-barcode').sheet1
            unprinted_codes = sheet.col_values(1)[1:]
            return [code for code in unprinted_codes if code and not (code.startswith("*") and code.endswith("#"))]
        else:
            print("Credentials not available.")
            return []

    def email_voucher_code(self, code):
        try:
            code_without_asterisk = code if not code.startswith("*") else code[1:]
            email_body = f"Hier is uw voucher code van het fietsstation: {code_without_asterisk}\n Geldig tot: {datetime.datetime.now() + datetime.timedelta(days=365)}"
            webbrowser.open(f"mailto:{self.email_input.text}?subject=Uw Voucher Code&body={email_body}")
        except Exception as e:
...
class ActivateScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.activate_input = TextInput(hint_text='Enter voucher code to activate', multiline=False, font_size=20)
        self.activate_button = Button(text='Activate Voucher', on_press=self.activate_voucher, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main, background_color=[0.7, 0.3, 0.3, 1], font_size=20)
...
class RedeemScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.redeem_input = TextInput(hint_text='Enter voucher code to redeem', multiline=False, font_size=20)
        self.redeem_button = Button(text='Redeem Voucher', on_press=self.redeem_voucher, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main, background_color=[0.7, 0.3, 0.3, 1], font_size=20)
...
class PrintScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.print_button = Button(text='Print Vouchers', on_press=self.print_vouchers, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main, background_color=[0.7, 0.3, 0.3, 1], font_size=20)
...
class CheckScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.check_input = TextInput(hint_text='Enter voucher code to check', multiline=False, font_size=20)
        self.check_button = Button(text='Check Voucher', on_press=self.check_voucher, background_color=[0.3, 0.5, 0.7, 1], font_size=20)
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main, background_color=[0.7, 0.3, 0.3, 1], font_size=20)
...
class CodeControlScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Initialize the codes attribute
        self.codes = []

        # Create a refresh button
        self.refresh_button = Button(text='Refresh', on_press=self.refresh_codes,
                                     background_color=[0.3, 0.7, 0.3, 1], font_size=14)

        # Create a TextInput widget for search
        self.search_input = TextInput(hint_text='Search code', multiline=False, font_size=14, size_hint=(0.8, None), height=30)
        ...
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main,
                                  background_color=[0.7, 0.3, 0.3, 1], font_size=14)
    ...
    def _refresh_codes(self, dt):
        try:
            ...
            # Update the codes attribute
            self.codes = codes_data
            
            # Update the code buttons to reflect the new data
            ...
            self.refresh_button.background_color = [0.3, 0.7, 0.3, 1]  # Revert button color back to original
    ...
    def update_code_buttons(self, instance=None, value=''):
        ...
        # Filter codes based on the search input
        search_text = self.search_input.text.lower()
        filtered_codes = [code for code in self.codes if search_text in code[0].lower()]

        # Create new buttons for each filtered code and add them to the layout
        for code_data in filtered_codes:
            code_button = Button(text=code_data[0], size_hint_y=None, height=40, font_size=14,
    ...
    def _activate_code(self, code, button):
        try:
            ...
            # Code to activate the code goes here

        except Exception as e:
            print("Error:", e)
        finally:
            button.background_color = [0.3, 0.7, 0.3, 1]  # Revert button color back to original
    ...
    def _redeem_code(self, code, button):
        try:
            ...
            # Code to redeem the code goes here
