...
class CodeControlScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.code_buttons_layout = BoxLayout(orientation='vertical', spacing=10, padding=20, size_hint_y=None)
        self.code_buttons_layout.bind(minimum_height=self.code_buttons_layout.setter('height'))

        scroll_view = ScrollView(size_hint=(1, None), size=(Window.width, Window.height))
        scroll_view.add_widget(self.code_buttons_layout)
    ...
    def update_code_buttons(self, *args):
        ...
        self.code_buttons_layout.clear_widgets()
        for code_data in self.codes:
            if search_text in code_data[0].lower():
                ...
                self.code_buttons_layout.add_widget(code_button)

    def show_code_info(self, code_data):
        print("Code data:", code_data)
        if len(code_data) >= 4:
            code, status, printed, expiry = code_data[:4]
            message = f"Code: {code}\nStatus: {'Active' if not code.startswith('*') else 'Inactive'}\nPrinted: {printed}\nExpiry: {expiry}"
            show_success_popup("Code Info", message)
        else:
            print("Invalid code data:", code_data)
            self.show_success_popup("Error", "Invalid code data")
    def switch_to_main(self, instance):
        self.manager.current = 'main'
