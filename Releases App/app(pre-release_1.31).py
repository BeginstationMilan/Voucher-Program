...
def make_printed(credentials_file_path=None):
    try:
        creds = get_credentials(credentials_file_path)
        if creds is None:
            raise Exception("Failed to load credentials. Exiting.")
        ...
        if len(unprinted_codes) < 12:
            raise ValueError("Not enough inactive codes available. Please generate more codes.")
        ...
        process_images(selected_codes)

    except Exception as e:
        print(f"An error occurred: {str(e)}")
        show_error_popup("Error", str(e))

def process_images(selected_codes):
    ...
    if not os.path.exists(final_image_filename):
        raise FileNotFoundError("Final image file not found.")
    ...
    with Image.open(final_image_filename) as final_img:
        ...
        generate_pdf([final_output_path, os.path.join(DEFAULT_DIR, 'standaard_full.png')], coupon_list_pdf)

def generate_pdf(image_paths, output_pdf_path):
    pdf_bytes = img2pdf.convert(image_paths)
    with open(output_pdf_path, "wb") as f:
        f.write(pdf_bytes)
    print("Coupon List PDF generated.")
    open_file_in_browser(output_pdf_path)
...
def open_file_in_browser(file_path):
    if os.path.exists(file_path):
        ...
        print(f"File opened successfully in browser: {file_path}")
    else:
...
class CodeControlScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.refresh_button = Button(text='Refresh', on_press=self._refresh_codes,
                                     background_color=[0.3, 0.7, 0.3, 1], font_size=14, size_hint=(1, None), height=40)

        self.search_input = TextInput(hint_text='Search code', multiline=False, font_size=18, size_hint=(1, None), height=50)
        ...
        self.back_button = Button(text='Back to Main Menu', on_press=self.switch_to_main,
                                  background_color=[0.7, 0.3, 0.3, 1], font_size=14, size_hint=(1, None), height=40)
